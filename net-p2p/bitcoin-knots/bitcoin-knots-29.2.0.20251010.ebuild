...
MyPV="$(ver_cut 1-2).knots$(ver_cut 4)"
[ "$(ver_cut 3)" = 0 ] || die

DESCRIPTION="Enhanced implementation of the Bitcoin cryptocurrency"
HOMEPAGE="https://bitcoinknots.org/"
SRC_URI="
	https://github.com/bitcoinknots/bitcoin/releases/download/v29.2.knots20251010/bitcoin-29.2.knots20251010.tar.gz
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DWITH_SYSTEM_LEVELDB=$(usex system-leveldb)
		-DBUILD_BITCOINCONSENSUS_LIB=$(usex libs)
		-DINSTALL_MAN=$(usex man)
		-DENABLE_TOR_SUBPROCESS=$(usex tor-subprocess)
		-DWITH_MINIUPNPC=$(usex upnp)
	)
	cmake_src_configure
}
...
src_compile() {
	if use gui ; then
		xsltproc --novalid --nonet --path src/qt/res/src "${FILESDIR}/flatten.xslt" src/qt/res/src/bitcoin.svg > bitcoinknots.svg
	fi
...
src_install() {
	...
	if use cli; then
		...
		doins contrib/completions/zsh/bitcoin-cli.zsh
	fi
	...
	if use daemon ; then
		...
		newconfd contrib/init/bitcoind.openrc bitcoind
